...
- Women mobile submenu thêm hai promo cards; Men mobile submenu thêm grid Best deals 2 cột với sale badge, ảnh variant, giá và quick-view icon theo style product card của demo.
- By activity và By feature bên trong Women/Men đã đổi từ accordion dropdown thành overlay panel riêng full-height, có animation trượt, header label và nút Back/Close riêng như demo. Nút Back hiện giữ panel trong DOM, chạy animation slide-out 0,3 giây rồi mới đóng để không mất hiệu ứng.
...
## 38. Giảm độ đậm item trong Pages submenu mobile (2026-08-09)
...
- Mega menu Women/Men dùng đúng padding cột, typography Archivo cho heading, khoảng cách banner/product và panel trắng mở theo đúng `scrollHeight` của nội dung, không còn bị trừ thừa `2px`. Ở sticky state, header trở thành khung trắng bo `16px`, cách mép trên `16px`; mega menu dùng padding ngang `30px` để bề rộng banner/card tiếp tục khớp demo.
- Lớp nền khi mở Women/Men, Search hoặc Cart phủ toàn viewport bằng `rgba(50, 50, 50, .5)` và `blur(20px)`. Pages vẫn không dùng lớp nền đen như demo. Cơ chế khóa cuộn bù đúng chiều rộng scrollbar và trừ phần bù khỏi sticky desktop bar, nên header/drawer không bị giật ngang và khung sticky vẫn giữ đúng bề rộng `1325px` ở viewport `1440px` khi mở panel.
